...
geosseislandsbyorders=function(order){
  ...
  bakedrangesord$abovemedianord=as.numeric(median(bakedrangesord$area)<bakedrangesord$area)
  
  tipstatesord=bakedrangesord$abovemedianord+1
...
roderesult=mcmapply(geosseislandsbyorders, orders, SIMPLIFY=F,mc.cores=20)

roderesult

save(roderesult,file="varmedianSecSSEmodeliiirode.RData")
